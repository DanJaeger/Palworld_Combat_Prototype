...
[CreateAssetMenu(fileName = "DogPatrolBehaviorStrategy", menuName = "Creatures/Strategies/Dog/Patrol")]
public class DogPatrolBehaviorStrategy : MovementStrategySO
{
    public override void StartMovement(CreatureStateMachine creature)
    {
        ...
        // Cada animal llamara a su propia instancia de GetRandomPatrolPoint()
    ...
    private void ApplySmoothRotation(CreatureStateMachine creature)
    {
        if (creature.Agent.velocity.sqrMagnitude > 0.1f)
        {
            ...
            // Cada transform.rotation es unico de la instancia del animal
